...
interface DossierAttendanceCardProps {
...
const RESPONSE_CHIP: Record<string, { label: string; bg: string; fg: string }> = {
  ...
  'not-recorded': { label: 'No Roll Call', bg: '#F3F4F6', fg: '#4B5563' }
};
...
const DossierAttendanceCard: React.FC<DossierAttendanceCardProps> = ({ attendance, loading }) => {
  const rate = attendance?.attendanceRate;

  return (
    <div style={{ ...dossierStyles.card, flexShrink: 0 }}>
      <div style={dossierStyles.cardHeader}>
        <span style={dossierStyles.cardHeaderText}>Attendance</span>
      </div>
      <div style={{ ...dossierStyles.cardContent, overflowY: 'visible' }}>
        ...
        ) : !attendance || attendance.totalEvents === 0 ? (
          <div style={{ color: '#64748B', fontSize: '14px', textAlign: 'center', padding: '24px 0' }}>
          ...
          <>
            ...
            <div style={{ display: 'flex', flexDirection: 'column' }}>
              {attendance.recent.map(item => {
                const chip = RESPONSE_CHIP[item.response] || RESPONSE_CHIP['not-recorded'];
                return (
                  <div
                    key={item.eventId}
                    style={{
...
export default DossierAttendanceCard;
