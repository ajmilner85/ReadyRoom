  ...
  try {
    ...
    if (eventId) {
      try {
        const { data: eventData, error: eventError } = await supabase
          .from('events')
          .select('track_qualifications, event_type, creator_call_sign, creator_board_number, creator_billet, participants, event_settings, syllabus_mission_id, cycle_id, reference_materials')
          .eq('id', eventId)
          .single();
  ...
  try {
    ...
    for (const publication of pendingPublications) {
      ...
      try {
        ...
        // Publish to all participating squadrons
        ...
        for (const squadronId of participatingSquadrons) {
          ...
          if (publishResult.success) {
            console.log(`[SCHEDULED-PUBLICATIONS] Successfully published event to squadron ${squadronId}, messageId: ${publishResult.messageId}`);
            publishedCount++;
            ...
            console.error(`[SCHEDULED-PUBLICATIONS] Failed to publish to squadron ${squadronId}:`, publishResult.error);
            errors.push({
