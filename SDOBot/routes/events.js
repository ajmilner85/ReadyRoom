 ...
 * EVENTS ROUTES
 *
 * PURPOSE: Handle event management endpoints for Discord publishing and editing
 *
 * ENDPOINTS:
 * - DELETE /api/events/:discordMessageId - Delete Discord event message and cleanup
 * - POST /api/events/publish - Publish new event to Discord
 * - PUT /api/events/:messageId/edit - Edit existing Discord event message
 * - GET /api/events/:eventId/attendance - Get attendance for an event
 * - GET /api/events/discord/:discordMessageId - Find event by Discord message ID
 * - POST /api/reminders/send - Send manual reminder to Discord
 *
 * DEPENDENCIES:
 * - Supabase client for database operations
 * - Discord bot functions for publishing, editing, deleting messages
 * - Countdown manager for event time updates
 * - Reminder processor for sending reminders
 *
 * USAGE:
 * Registered in main server: app.use('/api', eventsRoutes);
 */
...
router.put('/events/:messageId/edit', async (req, res) => {
  try {
...
router.get('/events/discord/:discordMessageId', async (req, res) => {
  try {
    const { discordMessageId } = req.params;
